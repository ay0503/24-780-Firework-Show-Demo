...
// Define the BurstStrategy interface
class BurstStrategy {
public:
  virtual ~BurstStrategy() = default;
  virtual std::vector<Particle> createBurst(double x, double y, float r,
                                            float g, float b) = 0;
};

class NormalBurst : public BurstStrategy {
public:
  std::vector<Particle> createBurst(double x, double y, float r, float g,
                                    float b) override {
    ...
    for (int i = 0; i < numParticles; ++i) {
      ...
      Particle particle(x, y, vx, vy, r, g, b, true);
...
class SpiralBurst : public BurstStrategy {
public:
  std::vector<Particle> createBurst(double x, double y, float r, float g,
                                    float b) override {
    std::vector<Particle> particles;
...
class DoubleBurst : public BurstStrategy {
public:
  std::vector<Particle> createBurst(double x, double y, float r, float g,
                                    float b) override {
    std::vector<Particle> particles = NormalBurst().createBurst(x, y, r, g, b);

    // Schedule the secondary burst for each particle
...
private:
  ...
  std::vector<Particle> burstParticles; // Particles created during the burst
...
public:
  Firework(double startX, double startY, double startVx, double startVy,
    ...
    this->burstParticles = strategy->createBurst(startX, startY, r, g, b);
  }
